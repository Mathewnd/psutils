...
typedef int (*list_token_fn)(struct pkill_options *opts, const char *token, const char *list_name);
...
static int parse_list(struct pkill_options *opts, const char *value, const char *list_name, list_token_fn token_fn) {
	char *copy = strdup(value);
	if (copy == NULL)
		return -1;
...
static int parse_parent_token(struct pkill_options *opts, const char *token, const char *list_name) {
	pid_t pid;
...
static int parse_process_group_token(struct pkill_options *opts, const char *token, const char *list_name) {
	pid_t pid;
...
static int parse_session_token(struct pkill_options *opts, const char *token, const char *list_name) {
	pid_t pid;
...
static int parse_effective_user_token(struct pkill_options *opts, const char *token, const char *list_name) {
	uid_t uid;
...
static int parse_real_user_token(struct pkill_options *opts, const char *token, const char *list_name) {
	uid_t uid;
...
static int parse_real_group_token(struct pkill_options *opts, const char *token, const char *list_name) {
	gid_t gid;
...
static int parse_terminal_token(struct pkill_options *opts, const char *token, const char *list_name) {
	(void)list_name;
...
static int parse_signal(const char *token, int *signal_number) {
	char *end;
	...
	if (!errno && *token && !*end && number > 0 && number < NSIG) {
	...
	if (strncasecmp(name, "SIG", 3) == 0)
		name += 3;
	else if (*name == '-' && strncasecmp(name + 1, "SIG", 3) == 0)
		name += 4;
	else if (*name == '-')
		++name;
...
static void usage(FILE *stream) {
	fprintf(stream,
	    "%s: usage: %s [-signal] [-fnovx] [-P ppidlist] [-g pgrplist]\n"
...
int parse_options(int argc, char **argv, struct pkill_options *opts) {
	...
	if (argc > 1 && argv[1][0] == '-' && argv[1][1] && parse_signal(argv[1], &opts->signal) == 0) {
		++argv;
		--argc;
	}

	opterr = 0;
	while ((ch = getopt(argc, argv, ":fnoP:g:s:u:U:G:t:vx")) != -1) {
		switch (ch) {
		case 'f':
